...
Логика — в `library`, `metadata`, `artwork`, `nfo`; здесь то, что в прежнем окне
делал `metaui.py`: состояние выбора, предпросмотр плана с записями .nfo и
картинок, подтверждения и запуск в фоне.
"""
...
def form():
    """Поля «Тип», «Название», «Год» — сервер помнит их, как прежнее окно."""
    t = _idle()
    data = replies.body()
...
def system_junk():
    ...
    Как в прежнем окне, два шага: поиск (на большой медиатеке — минуты), потом
