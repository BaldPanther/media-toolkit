...
.edl → главы), `chapters`, `trim`, `online`. Здесь то, что в прежнем окне делал
`app.py`: список серий с таймингами, настройки пропуска, «Применять к»,
подтверждения и запуск в фоне.
"""
