...
Обработчик кнопки не может посреди дела спросить «Продолжить?» и ждать ответа:
запрос должен вернуться сразу. Поэтому вопрос — это ответ сервера `{"ask": …}`:
страница показывает диалог и повторяет тот же запрос, добавив выбранное в
`answers`. Обработчик идёт по своему коду заново и на этот раз проходит вопрос,
забрав ответ из `answers`. Так логика и все тексты остаются на сервере, рядом с
проверками.
"""
