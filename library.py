...
# Запрещённые, которые заменяются пробелом: иначе слова слипаются.
...
def sanitize_name(name: str) -> str:
    ...
    Двоеточие и слэш заменяются пробелом, остальное запрещённое вырезается.
    Именно так это сделано в уже разложенной медиатеке: «Mission: Difficult» →
    «Mission Difficult», «Arrival/Departure» → «Arrival Departure». Вариант с
    « - » вместо двоеточия выглядит логичнее, но переименовал бы готовые файлы.

    Точки и пробелы в конце обрезаются — Windows такие имена не создаёт.
    """
    s = _TO_SPACE.sub(" ", name or "")
    s = _BAD_CHARS.sub("", s)
