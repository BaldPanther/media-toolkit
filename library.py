...
# Двоеточие-разделитель подзаголовка: опознаётся по пробелу следом. В «Re:Zero»
# и «9:30» двоеточие значит другое, и дефис там был бы неуместен.
_SUBTITLE_COLON = re.compile(r"\s*:\s+")
# Остальное запрещённое, что нельзя просто вырезать: иначе слова слипаются.
...
def sanitize_name(name: str) -> str:
    ...
    Двоеточие в названиях почти всегда отделяет подзаголовок, и на его месте
    ставится « - »: «The Lord of the Rings The Fellowship of the Ring» читается
    заметно хуже, чем через дефис. Само двоеточие оставить нельзя — Windows его
    в именах не допускает, а Finder рисует слэшем.

    Слэш заменяется пробелом: он стоит между словами пары
    («Arrival/Departure»), где дефис с пробелами выглядел бы неуместно.

    Точки и пробелы в конце обрезаются — Windows такие имена не создаёт.
    """
    s = _SUBTITLE_COLON.sub(" - ", name or "")
    s = _TO_SPACE.sub(" ", s)
    s = _BAD_CHARS.sub("", s)
    s = re.sub(r"\s{2,}", " ", s).strip()
    # Двоеточие в начале или в конце названия оставило бы висящий дефис.
